...
import { AlertTriangle, ArrowLeft, CheckCircle2, KeyRound, LockKeyhole, Network, Save, ShieldCheck } from "lucide-react";
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/it-support/settings", {
        ...
        body: JSON.stringify({ ...settings, intervalMinutes: Number(settings.intervalMinutes), adminToken }),
      });
  ...
  return <main className="its-shell its-settings-shell">
    ...
    <section className="its-settings">
      ...
      <form onSubmit={save}>
        ...
        <section className="its-settings-card its-token-card"><div><KeyRound aria-hidden="true" size={22} /><h2>ยืนยันตัวตนผู้ดูแล</h2><p>กรอกรหัสจาก <code>IT_SUPPORT_ADMIN_TOKEN</code> เพื่ออนุญาตให้เซิร์ฟเวอร์เขียนไฟล์ <code>.env</code></p></div><label>รหัสผู้ดูแลระบบ<input type="password" autoComplete="current-password" value={adminToken} onChange={(event) => setAdminToken(event.target.value)} required /></label></section>
